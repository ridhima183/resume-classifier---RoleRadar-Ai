...
import { getProfile, login, setAuthToken } from "../services/api";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (roleParam === "student" || roleParam === "recruiter") {
      ...
      setSelectedRoleState(roleParam);
    }
    ...
    if (oauthError) {
      const detail = hashParams.get("oauth_error_description");
      const providerLabel = oauthProvider || "OAuth";
      toast.error(
        detail
          ? `${providerLabel} login failed: ${detail}`
          : `${providerLabel} login failed. Check OAuth configuration.`
      );
    ...
    if (accessToken) {
      ...
      getProfile()
        .then((profileRes) => {
          ...
          setSelectedRoleState(profile.role || "student");
          toast.success(`${oauthProvider || "OAuth"} sign in successful!`);
          navigate(
            getPostLoginPath(profile.role || "student", profile.is_verified || false),
            { replace: true }
          );
        })
        .catch(() => {
          toast.error("OAuth sign in succeeded, but your profile could not be loaded.");
        })
        .finally(() => {
          window.history.replaceState(null, "", window.location.pathname + window.location.search);
        });
          ...
          <div className="mb-5 flex rounded-lg border border-gray-200 p-1 dark:border-gray-700">
            ...
            <button
              ...
              onClick={() => {
                ...
                setSelectedRoleState("recruiter");
              }}
              className={`flex-1 rounded-md px-3 py-2 text-sm font-semibold transition ${
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between">
                ...
                <Link
                  to="/forgot-password"
                  className="text-sm text-primary hover:text-indigo-600"
                >
                  Forgot password?
                </Link>
              </div>
